 ...
 * Load
 * -------------------------------------------
 * Load các file từ các vùng vào hệ thống tham gia xử lý
...
function load_view($name, $data_send = array()) {
    global $data;
    $data = $data_send;
    $path = MODULESPATH . DIRECTORY_SEPARATOR . get_module() . DIRECTORY_SEPARATOR . 'views' . DIRECTORY_SEPARATOR . get_controller() . 'Controller.php';
    if (file_exists($path)) {
        if (is_array($data)) {
